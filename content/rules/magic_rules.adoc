...
The power is considered a melee attack for all intents and purposes, typically a non-lethal one unless it deals damage.
It can be invoked to react to attacks, and can be reacted to in any way.

* *Sight*.
Targets must be visible and within range 8.
Unless otherwise specified, the power is _not_ considered a ranged attack, therefore relevant skills and rules don't apply.
In particular, the power always hits, no matter the distance to the target or whether the invoker moved.
However, the power can still be invoked to react to attacks, and can be reacted to, albeit only by countering.
...
The power is _not_ considered an attack, but can be used to react or reacted to, albeit only by countering.
