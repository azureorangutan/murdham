...
A common advantage granted by skills is proficiency at certain tasks.
This means you can reduce the difficulty of relevant actions.
...
* Attempting to perform the action more quickly than usual (in a round instead of a stretch).
If you fail, the action isn't successful, and you waste the original amount of time.

* Performing actions which are intrinsically almost impossible (scaling sheer surfaces, stealing large items...).

As usual, the GM is the final arbiter on whether a skill proficiency applies to a given situation or not, and you should feel free to apply this bonus even if not explicitly listed in the skill description.
...
It is rumoured to be the language spoken by foul demons.
...
Characters can't read, write, or perform anything but very simple calculations (with the aid of their fingers) unless they have a skill which grants them literacy: education is a privilege for the few!
